...
// Sin niebla: con la cámara alejada estaba lavando/aclarando toda la escena.
...
function rebuildArrow(charge){
  const len=.42 + charge*.72;
  const curveLift=.08 + charge*.10;
  ...
  arrowTip.quaternion.setFromUnitVectors(
...
function placeArrow(){
  ...
  const p=camera.position.clone().addScaledVector(dir,.85);
  p.y-=.10;
...
function spawnProjectile(typeIndex,dir,speed){
  ...
  if(typeIndex===0){
    geom=new THREE.SphereGeometry(.16,24,16);
    shape=new CANNON.Sphere(.16);
  }else if(typeIndex===1){
    geom=new THREE.BoxGeometry(.27,.27,.27);
    shape=new CANNON.Box(new CANNON.Vec3(.135,.135,.135));
  }else{
    geom=new THREE.TorusGeometry(.18,.065,12,28);
    geom.rotateX(Math.PI/2);
    shape=new CANNON.Sphere(.19);
  }
  ...
  const start=arrowRoot.position.clone().addScaledVector(dir,.45);
  mesh.position.copy(start);
  scene.add(mesh);

  const body=new CANNON.Body({
    mass:TYPES[typeIndex].mass,
    shape,
...
function currentAimDir(){
  ...
  const yaw=Math.sin(aimPhase)*THREE.MathUtils.degToRad(24);
