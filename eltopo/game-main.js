...
function setupRoom(code,host){
  ...
  A.chat.onMessage=(data,{peerId})=>{if(!isHost)return;const p=game.players[peerId];if(!p||p.alive===false||game.phase!=='discussion')return;const text=String(data?.text||'').trim().slice(0,500);if(!text)return;game.messages.push({id:crypto.randomUUID(),authorId:peerId,author:p.name,text,ts:Date.now(),round:game.round});syncAll()};
...
function renderAll(){const theme=THEMES[game.theme]||THEMES.escape;document.querySelectorAll('#groupName,#sidebarGroupName,#infoGroupName').forEach(e=>e.textContent=theme.name);$('groupSubtitle').textContent=`${Object.keys(game.players).length} participante${Object.keys(game.players).length===1?'':'s'} · ${peerCount()} peer${peerCount()===1?'':'s'} · código ${roomCode}`;$('sidebarPreview').textContent=game.phase==='lobby'?'Esperando jugadores':`Ronda ${game.round}/${game.totalRounds}`;renderPlayers();renderMessages();renderBanner();renderActions();updateVersion();$('infoCount').textContent=`${Object.keys(game.players).length} participantes`;$('participantTitle').textContent=`${Object.keys(game.players).length} participantes`}
...
function renderActions(){const t=$('actionTray');t.innerHTML='';t.classList.remove('hidden');if(!me()){t.innerHTML='<h4>Sincronizando sala…</h4>';return}if(me().alive===false){t.innerHTML='<h4>Fuiste expulsado. Seguís mirando la partida.</h4>';return}if(game.phase==='lobby'){t.innerHTML=`<h4>${isHost?'Compartí el código '+roomCode+'.':'Esperando que el administrador inicie.'}</h4>`;if(isHost){const b=button(`Iniciar partida (${Object.keys(game.players).length}/${MAX_PLAYERS})`,startGame,true);b.disabled=Object.keys(game.players).length<MIN_PLAYERS;t.appendChild(b)}return}if(game.phase==='avatar'){renderAvatars(t);if(isHost){const ready=Object.values(game.players).every(p=>p.avatar);const b=button(ready?'Comenzar ronda':'Esperando avatares…',beginRound,true);b.disabled=!ready;t.appendChild(b)}return}if(game.phase==='discussion'){t.innerHTML='<h4>Debatan libremente. Cualquier mensaje puede terminar en una captura.</h4>';if(isHost)t.appendChild(button('Cerrar conversación',()=>{game.phase='leak';sys('🔒 Conversación cerrada. El Topo está eligiendo qué filtrar.');syncAll()},true));return}if(game.phase==='leak'){if(myRole?.key==='topo'){t.innerHTML='<h4>🐀 Elegí qué mensaje filtrar.</h4>';game.messages.filter(m=>!m.system&&m.round===game.round).slice(-30).forEach(m=>{const d=document.createElement('div');d.className='leak-choice';d.innerHTML=`<div><b>${esc(game.players[m.authorId]?.name||m.author)}</b>: ${esc(m.text)}</div><button>📸 Filtrar esta captura</button>`;d.querySelector('button').onclick=()=>isHost?resolveLeak(m.id):A.leak.send({messageId:m.id},{target:hostId});t.appendChild(d)})}else t.innerHTML='<h4>Algo está pasando fuera del grupo…</h4>';return}if(game.phase==='investigation'){t.innerHTML='<h4>Discutan lo ocurrido y busquen al Topo.</h4>';if(isHost)t.appendChild(button('Crear encuesta de expulsión',openVote,true));return}if(game.phase==='voting'){renderVote(t);return}if(game.phase==='between'){t.innerHTML='<h4>El Topo sigue dentro del grupo.</h4>';if(isHost)t.appendChild(button('Siguiente ronda',beginRound,true));return}if(game.phase==='gameover')t.innerHTML=`<h4>${esc(game.result||'Fin de la partida')}</h4>`}
...
function startGame(){if(!isHost||Object.keys(game.players).length<MIN_PLAYERS)return;game.theme=$('themeSelect').value;game.totalRounds=Number($('roundsSelect').value)||5;game.antagonist=THEMES[game.theme].antagonist;const ids=shuffle(Object.keys(game.players)),topo=ids[0];game.topoId=topo;ids.forEach((id,i)=>{const role=i===0?{...TOPO_ROLE}:{...ROLES[(i-1)%ROLES.length]};if(id===selfId){myRole=role;showRole()}else A.role.send(role,{target:id})});sys('🔐 Los roles fueron repartidos en secreto.');game.phase='avatar';syncAll()}
...
function sendChat(){const input=$('messageInput'),text=input.value.trim();if(!text||game.phase!=='discussion')return;input.value='';if(isHost){game.messages.push({id:crypto.randomUUID(),authorId:selfId,author:myName,text:text.slice(0,500),ts:Date.now(),round:game.round});syncAll()}else A.chat.send({text:text.slice(0,500)},{target:hostId})}
...
$('sendBtn')?.addEventListener('click',sendChat);$('messageInput')?.addEventListener('keydown',e=>{if(e.key==='Enter')sendChat()});
$('groupInfoBtn')?.addEventListener('click',()=>$('infoPanel').classList.add('open'));$('closeInfoBtn')?.addEventListener('click',()=>$('infoPanel').classList.remove('open'));$('closeRoleBtn')?.addEventListener('click',()=>$('roleModal').classList.add('hidden'));
